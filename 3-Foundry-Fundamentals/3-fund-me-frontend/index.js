...
async function fund() {
  ...
  if (typeof window.ethereum !== "undefined") { // Check if MetaMask is installed
    const provider = new ethers.BrowserProvider(window.ethereum) // gets the rpc url from MetaMask
    ...
    try {
      ...
      }) // Sends the transaction to the contract using the metamask signer
      await transactionResponse.wait(1)
    } catch (error) {
      console.log(error)
    }
